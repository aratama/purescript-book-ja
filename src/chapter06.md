...
`Field`型クラスは、いくつかのより抽象的な**スーパークラス**が組み合わさってできています。これは、その型は`Field`型クラスの操作をすべてを提供しているわけではないが、その一部を提供する、というように抽象的に説明することができます。この型クラスは抽象的なすべてではないいくつかの数値演算子をサポートしています。例えば、自然数の型は加算および乗算については閉じていますが、減算については閉じていないため、この型は`Semiring`クラス(これは`Num`のスーパークラスです)のインスタンスですが、`Ring`や`Field`のインスタンスではありません。

スーパークラスについては、この章の後半で説明します。しかし、すべての数値型クラスの階層について述べるのはこの章の目的から外れているため、この内容に興味のある読者は`purescript-prelude`内の `Field`に関するドキュメントを参照してください。
