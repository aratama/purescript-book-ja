...
`Eq`型クラスはこの関数は2つの値が等しいかどうかをテストする ` eq`関数を定義しています。等値演算子(`==`)は `eq`の別名として定義されています。
...
`Field`型クラスはいくつかのスーパークラスから作られています。この型クラスは抽象的なすべてではないいくつかの数値演算子をサポートしています。例えば、自然数の型は、加算および乗算の際に閉じられるが、減法について閉じていないため、この型は `Semiring`クラスのインスタンスであり、 `Ring`や `Field`のインスタンスではありません。

スーパークラスはこの章の後半で説明します。しかしすべての数値型クラスの継承について述べるのはこの章の目的から外れているため、この内容に興味のある読者は `purescript-prelude`内の `Field`に関するドキュメントを参照してください。
...
演算子 `map`関数（エイリアス `<$>`）は関数をそのデータ構造まで「持ち上げる」(lift)ことができます。ここで「持ち上げ」という言葉の具体的な定義は問題のデータ構造に依りますが、すでにいくつかの単純な型についてその動作を見てきました。
...
`=>`シンボルを複数回使って複数の制約を指定できることに注意してください。複数の引数のカリー化関数を指定するのと同じです。しかし、2つの記号を混同しないように注意してください。

- `a -> b`は**型**`a`から**型**`b`への関数の型を表します。
- `a => b`は、型 `b`に**制約**`a`を適用します。
...
ここでは、この関数に `Int -> Int`または `Number -> Number`と注釈を付けることを考えられますが、PSCiでは一般的な型が `Semiring`で動作するため、 `Int`と `Number`の両方で関数を実行させることができます。
...
型クラスインスタンスが複数の他のインスタンスに依存する場合、それらのインスタンスは、 `=>`シンボルの左側で、カッコで区切り、コンマで区切る必要があります。
...
コンパイラは自動的にその排除を行うことはできず、 `streamString`インスタンスに引き渡すことはできません。しかし、型クラス定義にヒントを追加することで、コンパイラの助けになります。
...
プリミティブ型の `Hashable`インスタンスをいくつか書いてみましょう。まずは整数のインスタンスです。 `HashCode`は実際には単なるラップされた整数なので、これは簡単です - 私たちは `hashCode`ヘルパー関数を使うことができます。
...
パターンマッチングを使って `Boolean`値の単純なインスタンスを定義することもできます。
...
整数のインスタンスでは、 `Data.Char`の `toCharCode`関数を使って `Char`をハッシュするインスタンスを作成できます。
...
配列のインスタンスを定義するために、（要素型が `Hashable`のインスタンスでもあるならば）配列の要素に `hash`関数を `map`してから、 `combineHashes`関数の結果を使ってハッシュを左側に畳み込みます。
...
すでに書いたより単純なインスタンスを使用してインスタンスを構築する方法に注目してください。 `String`を `Char`の配列に変換することによって、新しい `Array`インスタンスを使って `String`インスタンスを定義しましょう。
