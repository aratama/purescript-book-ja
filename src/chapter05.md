...
これは型や関数をモジュール内で使用できるようにし、合わせて `Global.infinity`や `Math.max`などの修飾名を使用にできるようにします。これは重複したインポートをさけ、使用するモジュールを明確にすることに有効な方法です。

**注意**：同じモジュール名を修飾名に使用する場合には不要な作業です。 `import Math as M`などの短い名前を使用することが一般的に使用されています。
...
配列のリテラルパターンは、固定長の配列と一致させることができますが、PureScriptはこれらの方法で不変な配列を破壊するとパフォーマンスが低下する可能性があるため、不特定の長さの配列を一致させる手段を提供しません。 このようなマッチングをサポートするデータ構造が必要な場合は、 `Data.List`を使用することをお勧めします。 異なる動作に対して改善された漸近性能を提供する他のデータ構造が存在する。
...
> 
> 1. （やや難しい）配列リテラルパターンを使って１要素の配列の唯一のメンバーを抽出する関数 `fromSingleton`を書いてみましょう。１要素だけを持つ配列でない場合、関数は指定されたデフォルト値を返さなければなりません。 **ヒント**: この関数は `forall a. a -> Array a -> a`.という型を持っていなければなりません。
...
PureScriptコンパイラは、不完全なパターンマッチのために関数が全関数ではないことを検出できる場合にエラーを生成します。部分関数が安全である場合、 `unsafePartial`関数を使ってこれらのエラーを抑制することができます。上記の `unsafePartial`関数への呼び出しを取り除いた場合、コンパイラは次のエラーを生成します。
...
これは、値 `false`が定義されたどのパターンでも一致しないことを示しています。これらの警告には、複数の不一致のケースが含まれることがあります。
...
後で `=>`記号を含むより多くの型（それらは**型クラス**に関連しています）が見られます。しかし、今のところ、PureScriptは型システムを使って部分関数を追跡していること、開発者は型検証器にコードが安全であることを明示的に指示する必要があることを確認するだけで十分です。

コンパイラは、定義されたパターンが**冗長**であることを検出できる場合（すでに定義されたパターンに一致するケースのみ）でも警告を生成します。
