...
前章のモジュールに加えて、この章ではエントリポイントとなり、UIを描写する `Main`モジュールを使用します。

このプロジェクトをコンパイルするには、まずReactをインストールするため `npm install`を実行し、ビルドのため `pulp browserify --to dist/Main.js`を行います。このプロジェクトを実行するには、 `html/index.html`ファイルをウェブブラウザで開いてください。
...
最後の行の `pure`関数は予約語ではないことを思い出してください。 `pure`は実際にすべての `Applicative`関手について定義されている `Maybe`の `Applicative`関手に対する `Just`として定義されており、最後の行を `Just city`へ変更しても同じように正しく動きます。
...
ビルドツールPulpは、オプションを与えることで、アプリケーションの起動時に `main`計算を呼び出すためのJavaScriptコードを簡単に追加で生成できるようにしています。 `main`は `Eff`モナドでの計算であることが要求されます。
...
- `Render`型の作用の行は、いくつかの変わった作用を使用して、特定の関数からReactコンポーネントの状態へのアクセスを制限します。例えば、レンダリングの間、「refs」オブジェクトへのアクセスは `Disallowed`であり、コンポーネント状態へのアクセスは `ReadOnly`である。

`Main`モジュールは、住所録コンポーネントの状態型と初期状態を定義します。
...
すでに指摘したように、Reactクラスを作成するため、 `addressBook`は `createClass`と `spec`を使用します。ここから初期状態値と `Render`アクションを得ることができます。取得した `Render`アクションで何ができるのでしょうか？例えば、 `purescript-react`は以下のような単純なアクションを提供しています。
...
これで `addressBook`コンポーネントの定義を読むことができます。まず、現在のコンポーネントの状態を読み込みます。
...
- 名前 `ctx`は `ReactThis`を参照し、適切な状態を読み書きするために使用することができます。
- AppState内のレコードは、レコードバインダーを使用して一致しています。これには、**errors**フィールドのレコード同名利用も含まれます。便宜のためにステート構造体の様々な部分を明示しています。

`Render`は、DOMの意図された状態を表す `ReactElement`構造体を返さなければならないことを思い出してください。 `Render`アクションはいくつかの補助関数から定義されています。そのような補助関数の1つは `renderValidationErrors`です。これは `Errors`構造体を `ReactElement`の配列に変換します。
...
`purescript-react`では、 `ReactElement`は通常、単一のHTML要素を `div`のような関数を適用することで作成します。これらの関数は通常、属性の配列と子要素の配列を引数として取ります。しかし、ここでは `ul'`のようにダッシュ記号(', prime character)で終わる名前は属性配列を省略し、代わりにデフォルトの属性を使用します。
