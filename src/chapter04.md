...
実際には `map`の型は、２つの型を選択し汎用的に使用することができます。例えば、型を選択して `map`を以下のように扱うことができます。

```text
forall a b. (a -> b) -> Array a -> Array b
```

`map`関数に適用するときには `a`と `b`という２つの型を自由に選ぶことができると、この型は示しています。 `a`は元の配列の要素の型で、 `b`は目的の配列の要素の型です。もっと言えば、 `map`が配列要素の型を変化させても構わないということです。例えば、数値を文字列に変換するのにマップを使用することができます。
...
上記の例では、 `1 .. 5`という式は括弧で囲まれていましたが、実際にはこれは必要ありません。なぜなら、 `Data.Array`モジュールは、 `<$>`に割り当てられた優先順位より高い優先順位を `..`演算子に割り当てているからです。PureScriptでは予約語 `infix`を使用して後の数字である `8`として定義されていました。ここでは `<$>`の優先順位よりも高い優先順位を `(..)`に割り当てており、これは括弧を付け加える必要がないということです。
...
最後の行の `pure`関数は、 `PSCi`で評価するために型を明示しなければなりません。
...
`pure`と同じように、 `guard`関数がどのように動作するかを理解するために、 `PSCi`で通常の関数のように `guard`を適用してみましょう。 `guard`関数の型は、一般的な型であり `guard`は予約語**ではありません**.
