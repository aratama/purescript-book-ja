...
ここで、幻影型の引数 `a`は、属性キーと属性値が互換性のある型を持っていることを確認するために使われます。使用者は `AttributeKey a`を型の値を直接作成できないので(ライブラリで提供されている定数を介してのみ得ることができます)、すべての属性が正しくなります。
...
1. (難しい) 幻影型を使って真偽値 `true`、 `false`についての表現を最上位で定義することで、 `AttributeKey`が `disabled`や `chacked`のような**空の属性**を表現しているかどうかを符号化することができます。
...
1. (難しい) `ContentF`型を変更して、次の新しいアクションをサポートするようにしてください。
